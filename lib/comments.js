...
export default function (db, startFrom) {
	return {
		db,
		session: false,
		metadata: [],
			...
			if (!this.session) {
				...
				const processMeta = function (xml) {
					...
					if (xml.usermaps.usermap) {
						const addUsers = this.db.prepare(
							"insert or replace into `commenters` (`id`, `username`) values (:id, :user)"
						);
					...
					console.log(
						"Got metadata for " + this.metadata.length + " new comments."
					);
			...
			const processComments = function (xml) {
				...
				if (xml.comments.comment) {
					const addComment = db.prepare(
						"insert or replace into `comments` (`id`, `userid`, `entryid`, `parentid`, `date`, `subject`, `comment`) values (:id, :userid, :entryid, :parentid, :date, :subject, :comment)"
					);
					xml.comments.comment.forEach(function (c) {
						const id = parseInt(c["id"]);
					...
					console.log(
						"Downloaded " +
							this.numCommentDownloaded +
							" of " +
							this.metadata.length +
							" comments (" +
							Math.ceil(
								(this.numCommentDownloaded / this.metadata.length) * 100
							) +
							"%)..."
					);
				}
				if (
					this.maxid > this.commentDownloaded ||
					(!xml.comments.comment && this.maxid > this.commentDownloaded + 1000)
				) {
					this.downloadComments();
				} else {
					// we're done
					const lj = new ljReq();
					const splitSession = this.session.split(":");
					lj.send({
						mode: "sessionexpire",
						expire_id_num: splitSession[2]
					});
