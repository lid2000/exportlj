...
export default function (db, lastsync) {
	return {
		...
		sync: function () {
			...
			if (this.lastsync)
				obj["lastsync"] = this.lastsync.format("YYYY-MM-DD HH:mm:ss");
			const lj = new ljReq();
			const handleResponse = (data) => {
				...
				for (let i = 1; i <= count; i++) {
					...
					if (!this.lastsync || this.lastsync.valueOf() < lastsync.valueOf())
						this.lastsync = lastsync;
					if (!this.firstsync || this.firstsync.valueOf() > lastsync.valueOf())
						this.firstsync = lastsync;
				}
				...
				console.log(
					"Downloaded indexes for " +
						downloaded +
						" events, " +
						total +
						" more to go (" +
						Math.ceil((downloaded / (downloaded + parseInt(total))) * 100) +
						"%)..."
				);
				if (count == total) {
					// we're finished!
