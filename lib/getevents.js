...
export default function(db) {
	return {
		...
		processEntries: function(downloaded) {
			...
			const addProp = db.prepare("insert or replace into props (entryid, name, value) values (:entryid, :name, :value)");
			let newest;
			db.serialize(() => {
				...
				downloaded.forEach(function(entry) {
					...
					entry.props.forEach(function(prop) {
						addProp.run(makeParams({entryid: itemid, name: prop['name'], value: prop['value']}));
					});
