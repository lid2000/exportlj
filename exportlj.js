...
db.serialize(function() {
	...
	db.run("create table if not exists `moods` (`id` integer not null, `parent` integer not null, `name` text not null, primary key(`id`))");
	db.run("create table if not exists `userpics` (`name` text not null, `url` text not null, primary key(`name`))");
...
const exportLJ = function(lastsync) {
	...
	lj.send({mode: "login", getmoods: 0, getpickws: 1, getpickwurls: 1}, function(data) {
		...
		// do moods
		const addMood = db.prepare('insert or replace into moods (id, parent, name) values (:id, :parent, :name)');
		...
		const addUserpic = db.prepare('insert or replace into userpics (name, url) values (:name, :url)');
		addUserpic.run(makeParams({name: "__default_pic__", url: data.defaultpicurl}));
